using Test
using LogClustering
using LogClustering.ADWINs
using LogClustering.ADWINs: ADWIN, update!, width, last_drop_mean
...
@testset "Stats.ADWIN" begin
    ...
    @testset "detects an upward step change and adapts the mean" begin
        ...
        for _ in 1:500; update!(a, 10.0 + randn(rng)); end
        mean_before = mean(a)
        ...
        drop_at_change = NaN
        for _ in 1:300
            if update!(a, 50.0 + randn(rng)) && isnan(drop_at_change)
    ...
    @testset "detects a downward change (direction via last_drop_mean)" begin
        ...
        for _ in 1:500; update!(a, 50.0 + randn(rng)); end
        drop_at_change = NaN
        for _ in 1:300
            if update!(a, 10.0 + randn(rng)) && isnan(drop_at_change)
                drop_at_change = last_drop_mean(a)
            end
        end
